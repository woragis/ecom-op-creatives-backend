...
func main() {
	...
	for _, queue := range pipeline.AllQueues() {
		...
		if err := mq.Consume(q, func(d amqp.Delivery) error {
			return handleJob(context.Background(), runSvc, d.Body)
		}); err != nil {
			log.Fatalf("consume %s: %v", q, err)
		}
		log.Printf("worker stub consuming %s", q)
	}

	sig := make(chan os.Signal, 1)
	signal.Notify(sig, syscall.SIGINT, syscall.SIGTERM)
	<-sig
	log.Print("worker stub shutting down")
}

func handleJob(ctx context.Context, runs *creativerunsvc.Service, body []byte) error {
	...
	return runs.CompleteStepStub(ctx, stepID)
}
