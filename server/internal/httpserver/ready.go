package httpserver

import (
	"encoding/json"
	"log"
	"net/http"

	"github.com/woragis/ecom-op-creatives-backend/server/internal/apperrors"
	"github.com/woragis/ecom-op-creatives-backend/server/internal/platform/rabbitmq"
	"gorm.io/gorm"
)

func handleReady(db *gorm.DB, mq *rabbitmq.Client) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		sqlDB, err := db.DB()
		if err != nil {
			log.Printf("ready: sql db: %v", err)
			apperrors.WriteError(w, apperrors.UnavailableCause(apperrors.CodeReadyDatabasePingFailed, apperrors.MsgReadyDatabasePingFailed, err))
			return
		}
		if err := sqlDB.PingContext(r.Context()); err != nil {
			log.Printf("ready: db ping: %v", err)
			apperrors.WriteError(w, apperrors.UnavailableCause(apperrors.CodeReadyDatabasePingFailed, apperrors.MsgReadyDatabasePingFailed, err))
			return
		}
		if mq != nil {
			if err := mq.Ping(r.Context()); err != nil {
				log.Printf("ready: rabbitmq ping: %v", err)
				apperrors.WriteError(w, apperrors.UnavailableCause(apperrors.CodeReadyRabbitMQPingFailed, apperrors.MsgReadyRabbitMQPingFailed, err))
				return
			}
		}
		w.Header().Set("Content-Type", "application/json; charset=utf-8")
		_ = json.NewEncoder(w).Encode(map[string]string{"status": "ready"})
	}
}
